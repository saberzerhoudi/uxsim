...
def cli(verbose):
    ...
    Get started:
    1. uxsim setup-env    # Create .env file
    2. uxsim examples     # See usage examples  
    3. uxsim run -p persona.json  # Run simulation
    """
    if verbose:
...
def setup_env(output, force):
    ...
    try:
        ...
        env_template = """# UXSim Environment Variables
...
1. Mock environment simulation:
   uxsim run -p persona.json --policy cognitive_loop --environment mock

2. Web browser simulation (with GUI):
   export HEADLESS=false
   uxsim run -p persona.json --policy cognitive_loop --environment web_browser

3. Web browser simulation (headless):
   uxsim run -p persona.json --environment web_browser --start-url https://amazon.com

4. Component-based simulation:
   uxsim run -p persona.json --policy component --environment mock --max-steps 30

=== PERSONA CREATION ===

5. Create a persona:
   uxsim create-persona --name "Alex" --intent "buy wireless headphones" --age 28 --gender "male"

=== BATCH PROCESSING ===

6. Batch simulation:
   uxsim batch -c batch_config.yaml
...
Equivalent UXSim command:
uxsim run -p persona.json -o output --policy cognitive_loop --environment web_browser --llm-provider openai
